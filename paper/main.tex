...
\item \textbf{Generality of the failure mode} (Section \ref{sec:method-gini}): a gradient-magnitude check on the Theil index confirms the predicted vanishing-gradient-at-hub signature analytically (gradient $3.7{\times}10^{-9}$ at a synthetic hub point vs.\ $0.094$ at a genuinely varied selective point), but the full training-time diagnostic (does a Theil-regularized SAE actually collapse onto hub features the way Gini does) is still running at a corrected, sparsity-matched configuration and not yet reported.
