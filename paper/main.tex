...
We evaluate Rate-KL ($\rho{=}0.09$, two operating points) directly against Top-$k$, a Gated SAE, and Tuned $L_1$ on Fashion-MNIST, $N{=}1024$, at matched sparsity ($k$, and Rate-KL's $\rho$, both targeting $\approx$92 active features). Top-$k$ and Rate-KL values are 3-seed means $\pm$ std; Gated and Tuned $L_1$ are single-seed, included for broad positioning against the full method landscape rather than as replicated claims.

\begin{table*}[t]
\centering
\caption{All methods, Fashion-MNIST, $N{=}1024$, matched sparsity. Purity columns are the monosemanticity axis; Ablate/Clamp are the steering axis; MSE is the fidelity axis.}
\label{tab:positioning}
\begin{tabular}{@{}lcccccc@{}}
\toprule
Method & Sparsity & MSE & Mean purity & Top-10\% purity & Ablate & Clamp \\ \midrule
Tuned $L_1$ & 0.548 & \textbf{0.0021} & 0.053 & 0.000 & 0.025 & 0.050 \\
Gated SAE & 0.757 & 0.0038 & 0.093 & 0.003 & 0.024 & 0.049 \\
Top-$k$ & $0.910{\pm}.000$ & $0.0060{\pm}.0000$ & $0.265{\pm}.006$ & $0.028{\pm}.004$ & $0.0665{\pm}.0007$ & $0.1330{\pm}.0014$ \\
Gini & 0.910 & 0.0085 & 0.150 & 0.001 & 0.110 & 0.219 \\
Rate-KL ($\lambda{=}0.001$) & $0.906{\pm}.004$ & $0.0097{\pm}.0001$ & $0.599{\pm}.006$ & $0.429{\pm}.036$ & $0.0766{\pm}.0114$ & $0.1531{\pm}.0227$ \\
Rate-KL ($\lambda{=}0.01$) & $0.909{\pm}.005$ & $0.0159{\pm}.0004$ & $\mathbf{0.678{\pm}.002}$ & $\mathbf{0.525{\pm}.012}$ & $\mathbf{0.1414{\pm}.0049}$ & $\mathbf{0.2828{\pm}.0098}$ \\ \bottomrule
\end{tabular}
\end{table*}
...
\addplot[only marks, mark=triangle*, mark size=2.5pt, color=gray] coordinates {(0.000,0.025)};
\addplot[only marks, mark=square*, mark size=2.5pt, color=orange] coordinates {(0.003,0.024)};
\addplot[only marks, mark=diamond*, mark size=3pt, color=red] coordinates {(0.001,0.110)};
\addplot[only marks, mark=*, mark size=2.5pt, color=blue] coordinates {(0.028,0.0665)};
\addplot[only marks, mark=pentagon*, mark size=3pt, color=teal] coordinates {(0.429,0.0766)};
\addplot[only marks, mark=star, mark size=4pt, color=violet] coordinates {(0.525,0.1414)};
...
\addplot[only marks, mark=triangle*, mark size=2.5pt, color=gray] coordinates {(0.0021,0.025)};
\addplot[only marks, mark=square*, mark size=2.5pt, color=orange] coordinates {(0.0038,0.024)};
\addplot[only marks, mark=*, mark size=2.5pt, color=blue] coordinates {(0.0060,0.0665)};
\addplot[only marks, mark=diamond*, mark size=3pt, color=red] coordinates {(0.0085,0.110)};
\addplot[only marks, mark=pentagon*, mark size=3pt, color=teal] coordinates {(0.0097,0.0766)};
\addplot[only marks, mark=star, mark size=4pt, color=violet] coordinates {(0.0159,0.1414)};
...
\textbf{Magnitude:} We report per-feature magnitude directly only for the methods in Table \ref{tab:diagnosis}'s protocol (Gini, Top-$k$, Gated, Tuned $L_1$), where Gini's active-feature magnitude is $3\times$ Top-$k$'s. We did not separately log raw magnitude for Rate-KL in this evaluation pass; because ablation impact is exactly $|z_f|\cdot\lVert W_f\rVert$ and decoder norms are architecture-matched across methods, Rate-KL's steering advantage is indirect evidence of a magnitude advantage in the same direction, but we do not claim a directly measured number here and flag this as an evaluation gap to close.

\textbf{Fidelity: Rate-KL loses, clearly and worst-in-class.} MSE at $\lambda{=}0.01$ ($0.0159$) is not just worse than Top-$k$'s ($0.0060$, $2.7\times$) -- it is worse than \emph{every} method in Table \ref{tab:positioning}, including Gini ($0.0085$), the method this paper spends most of its length diagnosing as broken. This is the real, unglamorous cost of concentrating reconstruction responsibility onto fewer, more selective, higher-magnitude features, and Figure \ref{fig:fidelity-steer} is designed to make that cost impossible to miss.
...
\section{Investigating Feature Absorption: An Inconclusive Result}
\label{sec:absorption}
\citet{chanin2024absorption} identify \emph{feature absorption}: a concept's dedicated latent develops activation holes because a co-occurring, more specific latent has absorbed its direction, driven by sparsity pressure making the merge cheaper than firing two clean features. We hypothesized Rate-KL's per-feature rate constraint would resist this, since an absorbing latent's rate rises toward the union of the merged concepts' frequencies, individually penalized by Equation \ref{eq:ratekl}. We built a class-level adaptation (per-class logistic probes; a class's ``main latent'' is its highest-F1 firing predictor; an absorption instance is a sample where the main latent is silent but decoder-cosine-aligned other latents jointly carry the class-direction mass it typically carries), tested against a null control and a hub-contamination flag.

The result is inconclusive. MNIST's near-perfect class separability puts every method at the metric's floor. On CIFAR-10, weak linear pixel-space probes make the compensation criterion pass almost trivially, and three of five methods tested have every class hub-contaminated (10/10), making their numbers uninterpretable. Only Fashion-MNIST yields a trustworthy measurement, and there the pattern is mixed: a fix for a secondary rate-mismatch mechanism (target rate $\rho$ below the true class base rate, which independently manufactures absorption-like pressure) \emph{increased} measured absorption rather than decreasing it. We do not claim Rate-KL is more or less prone to Chanin-style absorption than Top-$k$; our evaluation proxy was not sound enough on two of three datasets to support a claim either way.
...
\item \textbf{Activity bimodality} (Section \ref{sec:bimodality}) is a real, dataset- and $\lambda$-dependent failure requiring an explicit validity gate; we do not yet have a general fix.
\item \textbf{A secondary rate-mismatch mechanism} can manufacture absorption-like pressure when a uniform target $\rho$ is below a concept's natural frequency; our attempted fix did not work (Section \ref{sec:absorption}).
\item \textbf{Magnitude was not directly measured for Rate-KL} in this evaluation pass (Section \ref{sec:positioning}); closing this gap is immediate future work.
\item \textbf{Generality of the failure mode} (Section \ref{sec:method-gini}) is argued structurally, not tested empirically beyond the Gini instance -- we have not run other distribution-shape penalties (e.g.\ Theil index) to confirm the same failure recurs.
\end{itemize}
