...
for epoch in range(num_epochs):
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        # FLIPPED SIGN: SUBTRACT Gini to ENCOURAGE sparsity (high inequality)
        lambda_sparse = 0.1  # tunable hyperparameter to control sparsity
        total_loss = reconstruction_loss - lambda_sparse * gini_loss  # NOTE THE MINUS SIGN
        ...
        if batch_idx % 10 == 0:
            ...
            print(f'[SPARSE] Epoch [{epoch+1}/{num_epochs}], Batch [{batch_idx+1}/{len(train_loader)}], '
                  ...
                  f'Weighted Gini: {(-lambda_sparse * gini_loss).item():.4f}')  # Negative because we subtract
...
with torch.no_grad():
    ...
    for data, _ in test_loader:
        ...
        total_loss = reconstruction_loss - lambda_sparse * gini_loss  # SPARSE version (subtract)
